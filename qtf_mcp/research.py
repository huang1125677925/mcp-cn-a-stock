...
def is_stock(symbol: str) -> bool:
    if (
        ...
        or symbol.startswith("SZ00")
        or symbol.startswith("SZ30")
    ):
        return True
    return False
...
def build_basic_data(fp: TextIO, symbol: str, data: Dict[str, ndarray]) -> str:
    ...
    if is_stock(symbol):
        print(
            f"- 市盈率: {(data['CLOSE2'][-1] / data['EPS'][-1]) * fin_ratio:.2f}",
            file=fp,
        )
        print(
            f"- 市净率: {data['CLOSE2'][-1] / data['EPSU'][-1] * fin_ratio:.2f}",
            file=fp,
        )
...
def today_volume_est_ratio(data: Dict[str, ndarray], now: int = 0) -> float:
    ...
    now_dt = (
        ...
        if now == 0
        else datetime.datetime.fromtimestamp(now / 1e9)
    )
...
def build_technical_data(fp: TextIO, symbol: str, data: Dict[str, ndarray]) -> str:
    ...
    date = [
        datetime.datetime.fromtimestamp(d / 1e9).strftime("%Y-%m-%d")
        for d in data["DATE"]
    ]
    columns = [
        ("日期", date),
    ...
    for i in range(-1, max(-len(date), -31), -1):
        print(
            ...
            + date[i]
            + "|"
            + " | ".join([f"{c[1][i]:.2f}" for c in columns[1:]])
            + " |",
            file=fp,
        )
    print("", file=fp)
